...
export class KorpusFreqDB implements IFreqDB {
    ...
    private loadData(
        ...
        type: string,
        ci: boolean
    ): Observable<HTTPDataResponse> {
        const words = value.split(' ');
        const fcritLocation = List.init(
            words.length > 1
    ...
    findQueryMatches(
        ...
        return forkJoin([
            this.loadResources(),
            this.loadData(word, ':form:attr:cnc:w:word', true),
        ]).pipe(
            concatMap(([rsc, data]) =>
                rxOf(
                    ...pipe(
                        ...
                        List.reduce<
                            DataBlock,
                            Array<[string, string, Array<PosItem>]>
                        >((acc, curr) => {
                            if (
                                word.toLowerCase() ===
                                    curr._name.toLowerCase() &&
                                curr[fcrit]
                            ) {
                                const lemma = List.map(
                                    (slot) =>
                                        slot._fillers[0][
                                            ':form:attr:cnc:w:lemma'
                                        ],
                                    curr._slots
                                ).join(' ');
                    ...
                    (acc, curr) => {
                        if (curr[fcrit]) {
                            ...
                            if (
                                ...
                                posTagsEqual(
                                    pos,
                                    List.map((v) => v.value, wordPos)
                                )
                            ) {
                                const total =
                                    resources.data[0].resources.corpus[
                                        this.normPath
                                    ].params.size_tokens;
                                const ipm = (1000000 * curr[fcrit]) / total;
                                // aggregate items with identical word
                                const ident = List.findIndex(
                                    (obj) => obj.word === curr._name,
                                    acc
                                );
                                if (ident > -1) {
                                    acc[ident].abs += curr[fcrit];
                                    acc[ident].ipm += ipm;
                                    acc[ident].flevel = calcFreqBand(
                                        acc[ident].ipm
                                    );
                                    return acc;
                                } else {
    ...
    getSourceDescription(
        ...
        return this.sourceInfoApi
            ...
            : this.loadResources().pipe(
                  map((res) => ({
                      tileId: -1,
                      title: res.data[0].resources.corpus[this.normPath].label[
                          uiLang
                      ],
                      description:
                          res.data[0].resources.corpus[this.normPath]
                              .description,
                      author: '',
                      structure: {
                          numTokens:
                              res.data[0].resources.corpus[this.normPath].params
                                  .size_tokens,
                      },
                  }))
              );
