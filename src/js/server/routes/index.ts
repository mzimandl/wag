...
function jsonOutputError(res: Response, err: Error): void {
    if (err instanceof ServerHTTPRequestError) {
        ...
        res.status(
            err.status >= 400 ? err.status : HTTP.Status.InternalServerError
        ).send({
...
function langSwitchError({
    req,
    res,
    services,
    messageKey,
    messageArgs,
}: LangSwitchErrorArgs): void {
...
export const wdgRouter = (services: Services) => (app: Express) => {
    ...
    app.get(HTTPAction.MAIN, (req, res, next) => {
        res.redirect(
            301,
            urlJoin(services.clientConf.rootUrl, HTTPAction.SEARCH)
        );
    });
    ...
    app.get(HTTPAction.GET_LEMMAS, (req, res, next) => {
        const [, appServices] = createHelperServices(
            services,
            getLangFromCookie(req, services)
        );
        logAction({
    ...
    app.post(HTTPAction.SET_UI_LANG, (req, res, next) => {
        ...
        if (!cookieName) {
            ...
            res.cookie(cookieName, services.toolbar.importLangCode(newUiLang), {
    ...
    app.get(`${HTTPAction.SEARCH}:query`, (req, res, next) => {
        ...
        if (langOverride.length > 0) {
            ...
            if (!cookieName) {
                ...
                Dict.hasKey(langOverride[0], services.serverConf.languages)
            ) {
                res.cookie(
                    cookieName,
        ...
        importQueryRequest({
            ...
            .subscribe({
                next: (conf) => {
                    ...
                    res.send(
                        JSON.stringify({
                            resultURL: appServices.createActionUrl(
                                `${HTTPAction.SEARCH}${conf.queries[0].word}`
                            ),
                            error: null,
                        })
                    );
    ...
    app.get(HTTPAction.WORD_FORMS, (req, res) => {
        ...
        const viewUtils = new ViewUtils<GlobalComponents>({
            ...
            staticUrlCreator: (path) =>
                services.clientConf.runtimeAssetsUrl + path,
            ...
            if (freqDb === undefined) {
                observer.error(
                    new ServerHTTPRequestError(
                        HTTP.Status.BadRequest,
                        `Frequency database for [${req.query.lang}] not defined`
                    )
                );
            }
            const posAttr = getQueryValue(
                req,
                'mainPosAttr'
            )[0] as MainPosAttrValues;
            observer.next({
                domain: getQueryValue(req, 'domain')[0],
                word: getQueryValue(req, 'word')[0],
                lemma: getQueryValue(req, 'lemma')[0],
                pos: List.map(
                    (v) => importQueryPos(v, posAttr),
                    getQueryValue(req, 'pos')
                ),
                posAttr,
            });
        })
    ...
    app.post(HTTPAction.SET_THEME, (req, res) => {
        ...
        res.cookie(THEME_COOKIE_NAME, req.body.themeId, {
            expires: new Date(Date.now() + 5 * 365 * 24 * 3600 * 1000),
        });
        res.redirect(req.body.returnUrl);
    });
    ...
    app.get(HTTPAction.SOURCE_INFO, (req, res) => {
        ...
        new Observable<IFreqDB>((observer) => {
            ...
            .pipe(
                concatMap((db) => {
                    return db.getSourceDescription(
                        uiLang,
                        getQueryValue(req, 'corpname')[0]
                    );
                })
            )
            .subscribe({
    ...
    app.use(function (req, res, next) {
        ...
        const error: [number, string] = [
            ...
            viewUtils.translate('global__action_not_found'),
        ];
        errorPage({ req, res, uiLang, services, viewUtils, error });
    });
};
